...
```{julia}
scatter([p[1] for p in X], [p[2] for p in X]; markersize=4)
```

Now take $L$ as a hundred farthest-point-sampled landmarks and let's create the ball mapper of $X$ with radius $\varepsilon = 0.25$:

```{julia}
L  = farthest_points_sample_ids(X, 100)
mp = ball_mapper(X, L, 0.25);
```
